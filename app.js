...
// ==========================================
// ==========================================
// 8. Real Excel (.xlsx) Export Handlers (Maktab Standart Formati)
// ==========================================
function buildSchoolTimetableSheet(classesList, titleText) {
    ...
    // Col 0: Hafta kuni, Col 1: Soat (№)
    cols.push({ wch: 14 }); // Kun
    cols.push({ wch: 5 });  // №

    const totalCols = 2 + classesList.length * 2;

    // Row 0: Top Approval Header
    const row0 = new Array(totalCols).fill('');
    if (totalCols > 4) {
        row0[totalCols - 3] = "TASDIQLAYMAN";
    }
    data.push(row0);
    ...
    if (totalCols > 4) {
        row1[totalCols - 3] = "Maktab direktori: ____________";
    }
    data.push(row1);
    ...
    row3[2] = titleText || "2026-2027 O'QUV YILI UCHUN DARS JADVALI";
    data.push(row3);
    if (totalCols > 6) {
    ...
    // Row 5 & 6: Header Rows (Sinf nomlari & Fan / O'qituvchi / Xona)
    const headerRow1 = ["Hafta kuni", "№"];
    const headerRow2 = ["", ""];
    ...
    classesList.forEach((c, idx) => {
        ...
        headerRow1.push(`${c.name}${supLabel}`, "");
        headerRow2.push("Fan / O'qituvchi", "Xona");
        
        merges.push({ s: { r: 5, c: colStart }, e: { r: 5, c: colStart + 1 } });
        cols.push({ wch: 22 }); // Fan va O'qituvchi
        cols.push({ wch: 7 });  // Xona
    });
...
function exportToExcel() {
    if (typeof XLSX !== 'undefined') {
        ...
        // 1. Asosiy Umumiy Dars Jadvali (Aynan Maktab namunasi shaklida)
        const sortedClasses = [...state.classes].sort((a, b) => (a.shift - b.shift) || a.name.localeCompare(b.name, undefined, { numeric: true }));
        const wsMaster = buildSchoolTimetableSheet(sortedClasses, "UMUMIY MAKTAB DARS JADVALI");
        XLSX.utils.book_append_sheet(wb, wsMaster, "Dars_Jadvali_Asosiy");
        ...
        // 4. O'qituvchilar Dars Jadvali va Bo'sh Kunlari
        ...
        state.teachers.forEach(t => {
            ...
            const tHeader = ["Dars Soati", "1-soat", "2-soat", "3-soat", "4-soat", "5-soat", "6-soat"];
            teachersScheduleData.push(["Hafta kuni", ...tHeader.slice(1)]);
