...
// Universal Online Cloud Fetch (Netlify, Vercel, Node, Custom Server)
async function fetchOnlineScheduleData(silent = false) {
    if (!window.location.protocol.startsWith('http')) return false;

    const endpoints = ['/.netlify/functions/schedule', '/api/schedule'];
            ...
            if (response.ok) {
                ...
                if (contentType.includes('application/json')) {
                    ...
                    if (res && res.success && res.data && res.data.classes && res.data.classes.length > 0) {
                        ...
                        state = res.data;
                        if (res.auth) {
                            authState.adminUsername = res.auth.adminUsername || authState.adminUsername;
                            authState.adminPassword = res.auth.adminPassword || authState.adminPassword;
                        }
                        ensureKelajakSubject();
...
async function pingPresence() {
    ...
    const endpoints = ['/.netlify/functions/presence', '/api/presence'];
    for (const url of endpoints) {
        try {
            const res = await fetch(url, {
                method: 'POST',
...
// ==========================================
// 8. Real Excel (.xlsx) Export Handlers
// ==========================================
function exportToExcel() {
    if (typeof XLSX !== 'undefined') {
        const wb = XLSX.utils.book_new();

        // SHEET 1: Umumiy Maktab Jadvali
        const masterData = [];
        masterData.push(["MAKTAB DARS JADVALI (2 SMENALI: 1-smena 08:00–13:10, 2-smena 12:25–17:20)"]);
        masterData.push(["Izoh: Har bir sinfda Dushanba 1-soat 'Kelajak soati' (Sinf rahbari). 1-smena 6-soat va 2-smena 1-soat 12:25–13:10 da bir xil vaqtda o'tiladi."]);
        masterData.push([]);

        const headerRow = ["Smena va Soat", "Vaqt"];
        DAYS.forEach(day => {
            state.classes.forEach(c => {
                const sup = c.supervisorId ? state.teachers.find(t => t.id === c.supervisorId) : null;
                const supText = sup ? ` (Rahbar: ${sup.name})` : '';
                headerRow.push(`${day} [${c.name}${supText}]`);
            });
        });
        masterData.push(headerRow);

        ALL_DAY_SLOTS.forEach(slotInfo => {
            const row = [slotInfo.label, slotInfo.time];
            DAYS.forEach((day, dayIndex) => {
                state.classes.forEach(c => {
                    const matching = state.lessons.find(l => l.day === dayIndex && l.classId === c.id && getLessonAbsoluteSlot(l) === slotInfo.slot);
                    if (matching) {
                        const subj = state.subjects.find(s => s.id === matching.subjectId);
                        const teacher = state.teachers.find(t => t.id === matching.teacherId);
                        row.push(`${subj ? subj.name : ''} (${teacher ? teacher.name : ''})`);
                    } else {
                        row.push("-");
                    }
                });
        ...
        XLSX.utils.book_append_sheet(wb, wsMaster, "Umumiy_Jadval");

        // SHEET 2: O'qituvchilar Dars Jadvali
        ...
        state.teachers.forEach(t => {
            ...
            const tHeader = ["Smena & Soat", "Vaqt", ...DAYS];
            teachersScheduleData.push(tHeader);

            ALL_DAY_SLOTS.forEach(slotInfo => {
                const tRow = [slotInfo.label, slotInfo.time];
                DAYS.forEach((day, dayIndex) => {
                    if (t.offDays && t.offDays.includes(dayIndex)) {
                        tRow.push("BO'SH KUN (DAM OLISH)");
                    } else {
                        const match = state.lessons.find(l => l.day === dayIndex && l.teacherId === t.id && getLessonAbsoluteSlot(l) === slotInfo.slot);
                        if (match) {
                            const c = state.classes.find(cls => cls.id === match.classId);
                            const s = state.subjects.find(sub => sub.id === match.subjectId);
                            tRow.push(`${s ? s.name : ''} - ${c ? c.name : ''} sinf`);
                        } else {
                            tRow.push("-");
                        }
                    }
        ...
        // SHEET 3: Sinflar va Sinf Rahbarlari Ro'yxati
        const classesList = [
        ...
        // SHEET 4: O'qituvchilar Ro'yxati va Yuklamasi
        const teachersList = [
            ["F.I.O.", "O'qitadigan Fanlari", "Sinf Rahbarligi", "Haftalik Soat", "Bo'sh Kunlari", "Telefon"]
        ];
        ...
        state.teachers.forEach(t => {
            ...
            teachersList.push([
                t.name,
                subjStr,
                supClass ? `${supClass.name} sinf rahbari` : 'Yo\'q',
                t.weeklyHours || 30,
                getOffDaysText(t.offDays),
                t.phone || ''
            ]);
        });

        const wsTeachersList = XLSX.utils.aoa_to_sheet(teachersList);
        XLSX.utils.book_append_sheet(wb, wsTeachersList, "Oqituvchilar_Royxati");

        XLSX.writeFile(wb, `Maktab_Dars_Jadvali_${new Date().toISOString().slice(0, 10)}.xlsx`);
        showToast("Haqiqiy Excel fayli (.xlsx) yuklab olindi!");
    } else {
...
function setupEventListeners() {
    ...
    document.getElementById('form-login').addEventListener('submit', (e) => {
        ...
        if (u === authState.adminUsername && p === authState.adminPassword) {
            ...
            document.getElementById('login-error-text').textContent = "Login yoki parol noto'g'ri! (Dastlabki parol: 123)";
        }
    });
    ...
    document.getElementById('form-change-password').addEventListener('submit', (e) => {
        ...
        showToast("Administrator paroli muvaffaqiyatli o'zgartirildi!");
    });
