...
// ==========================================
// Persistent Storage Layer (IndexedDB + Multi-Layer LocalStorage)
// ==========================================
const IDB_NAME = 'EduScheduleDB_v1';
const IDB_STORE = 'school_store';
...
async function saveToPersistentStorage(scheduleData, authData) {
    try {
        const db = await openIDB();
        if (db) {
            const tx = db.transaction(IDB_STORE, 'readwrite');
            const store = tx.objectStore(IDB_STORE);
            if (scheduleData) store.put(scheduleData, 'master_schedule');
            if (authData) store.put(authData, 'auth_data');
        }
    } catch (e) {}
}
...
async function loadFromPersistentStorage() {
    try {
        ...
        return new Promise((resolve) => {
            ...
            const store = tx.objectStore(IDB_STORE);
            const req = store.get('master_schedule');
...
// Universal Online Cloud Fetch (Render Server with Auto-Healing)
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (contentType.includes('application/json')) {
                ...
                const serverUpdated = res.updatedAt ? new Date(res.updatedAt).getTime() : 0;

                // 1. Agar serverda ma'lumot bo'lsa va u yangiroq bo'lsa -> serverdan yuklab olamiz
                if (res && res.success && res.data && res.data.classes && res.data.classes.length > 0) {
                    if (serverUpdated >= localUpdated || !state.lessons || state.lessons.length === 0) {
                        ...
                        localStorage.setItem('eduschedule_permanent_backup', JSON.stringify(state));
                        saveToPersistentStorage(state, authState);
                        ...
                        // Local ma'lumotlar yangiroq (Server uxlagan yoki qayta ishga tushgan) -> Serverni tiklaymiz (Auto-Heal)
                        syncWithServer();
                        updateCloudStatusBadge("online");
                        return true;
                    }
                } else {
                    // Server bo'sh (Render qayta yuklangan) -> Agar bizda ma'lumot bo'lsa, serverga yuklab uni darhol tiklaymiz!
...
function loadState() {
    const saved = localStorage.getItem(MASTER_DB_KEY) || 
                  localStorage.getItem('eduschedule_permanent_backup') ||
    ...
    // Try background load from IndexedDB if localStorage was wiped
    loadFromPersistentStorage().then(idbState => {
        if (idbState && idbState.classes && idbState.classes.length > 0) {
            const idbTime = idbState.updatedAt ? new Date(idbState.updatedAt).getTime() : 0;
            const curTime = state.updatedAt ? new Date(state.updatedAt).getTime() : 0;
            if (idbTime > curTime) {
                state = idbState;
                localStorage.setItem(MASTER_DB_KEY, JSON.stringify(state));
                refreshAllViews();
    ...
    // Fetch live data from server/cloud with auto-heal
...
// ==========================================
// ==========================================
// 8. Real Excel (.xlsx) Export Handlers (Aynan Namunadagi Format: Fan va O'qituvchi Alohida Satrlarda)
// ==========================================
function buildSchoolTimetableSheet(classesList, titleText) {
    ...
    // Top rows (Rasmdagi kabi bo'sh joy va sarlavha qatorlari)
    ...
    // Header Row (9-qator: Sinf nomlari) - Rasmdagidek to'g'ridan-to'g'ri sinf nomlari bilan
    const headerRowIdx = data.length;
    const headerRow = ["Kun", "№"];
    classesList.forEach((c, idx) => {
        const colStart = 2 + idx * 2;
        headerRow.push(c.name, "");
        merges.push({ s: { r: headerRowIdx, c: colStart }, e: { r: headerRowIdx, c: colStart + 1 } });

        cols.push({ wch: 22 }); // Fan va O'qituvchi katagi kengligi
        cols.push({ wch: 6 });  // Xona katagi kengligi
    });
    data.push(headerRow);
    ...
    DAYS.forEach((dayName, dayIndex) => {
        ...
        for (let p = 1; p <= maxPeriods; p++) {
            const subjRowIdx = data.length;
            const rowSubject = new Array(totalCols).fill('');
            const rowTeacher = new Array(totalCols).fill('');

            // Kun nomi faqat kunning 1-soatida yoziladi (keyin vertikal birlashtiriladi)
            if (p === 1) {
                rowSubject[0] = dayName;
            }

            // Dars soati raqami (1, 2, 3...)
            rowSubject[1] = p;
            rowTeacher[1] = "";

            // Har bir sinf uchun fan (yuqori satr) va o'qituvchi (pastki satr)
            classesList.forEach((c, idx) => {
                const colStart = 2 + idx * 2;
                ...
                if (match) {
                    ...
                    // 1-satr: FAN NOMI
                    rowSubject[colStart] = subjName;
                    // XONA (2 satr balandligida birlashtiriladi)
                    rowSubject[colStart + 1] = roomName;

                    // 2-satr: O'QITUVCHI F.I.O
                    rowTeacher[colStart] = teacherName;
                    rowTeacher[colStart + 1] = "";
                } else {
                    rowSubject[colStart] = "";
                    rowSubject[colStart + 1] = "";
                    rowTeacher[colStart] = "";
                    rowTeacher[colStart + 1] = "";
                }

                // Xona katagini 2 satr bo'yicha vertikal birlashtirish
                merges.push({ s: { r: subjRowIdx, c: colStart + 1 }, e: { r: subjRowIdx + 1, c: colStart + 1 } });
            });

            // Dars soati raqami katagini (№) 2 satr bo'yicha vertikal birlashtirish
            merges.push({ s: { r: subjRowIdx, c: 1 }, e: { r: subjRowIdx + 1, c: 1 } });

            data.push(rowSubject);
            data.push(rowTeacher);
        }

        const dayEndRow = data.length - 1;
        // Hafta kuni ustunini kunning barcha darslari bo'yicha to'liq vertikal birlashtirish
